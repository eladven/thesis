...
$$\dot{V}(t) = 2\left(z_1^\top\mathcal{A}(\omega_1)z_1+z_2^\top\mathcal{A}(\omega_2)z_2 -2z_1^\top\mathcal{B}(\delta)z_2 + T_mz_1^\top e_3+ T_mz_2^\top e_3\right)$$ 
explicitly,
$$
\begin{aligned}
\dot{V}(t) &= 2\left(-R_{tot}\left(i_{d1}^2+i_{q1}^2+i_{d2}^2+i_{q2}^2 \right) -D_p\left( \omega_1^2+\omega_2^2\right)\right) \\
& +4R_L\left( -\cos(\delta)i_{d1}i_{d2}-\cos(\delta)i_{q1}i_{q2}-\sin(\delta)i_{q1}i_{d2}+\sin(\delta)i_{q2}i_{d1}\right)
\\ & +2T_m(\omega_1+\omega2).
\end{aligned}
$$ 
...
According to Schur's Inequality if the therm $A_1-B^*A_1^{-1}B$ is a positive definite matrix, then it implies that  the matrix $A$ is a positive definite matrix. 
$$
\begin{aligned}
...
R_L\cos(\delta) & R_L \sin(\delta)  \\ 
-R_L \sin(\delta) & R_L\cos(\delta)  
\end{array}\right]\left[\begin{array}{cc}
R_{tot}^{-1} & 0  \\ 
0 & R_{tot} ^{-1} 
\end{array}\right]\left[\begin{array}{cc}
...
$$
\begin{aligned}
\omega_{2}i_{q2}-\omega_{1}i_{q1}= &\frac{1}{2}\left[(\omega_{2}+\omega_{1})(i_{q2}-i_{q1})+(\omega_{2}-\omega_{1})(i_{q2}+i_{q1})\right] \\ =&\frac{x_{w}e_{q}+e_{w}x_{q}}{2}
\end{aligned}
$$
...
\begin{proof}
We start by calculating $\frac{\partial F(e,x)}{\partial e}(0,x)$ for the TICSG system. 
$$
\begin{aligned}
...
Note, that the linearization of $F$ with respect to $e$, has the same structure of the linearization of a system of a single generator connected to an infinite bus around it equilibrium point  (see \cite[equation 3.4]{NaWe:15}). 

The linearization of a single generator connected to an infinite bus around an equilibrium point $(i^e_d, i^e_q, \omega_g, \delta^e)$ is
$$
\left[\begin{array}{c}
\dot{x_1} \\
\dot{x_2 }\\
...
For $x_\omega = 2 \omega_g$, $x_q = 2 i_q^e$, $x_d = i_d^e$, $V \cos(\tilde{\delta}^e) = -R_L 2 i_q^e$ and $V \cos(\tilde{\delta}^e) = -R_L 2 i_q^e$ we get the same dynamics, so the linearization of the F with respect to $e$ behaves like the linearization of a single generator connect to an infinte bus, with doubled currents and angular velocity, and with voltage that is equal to the load resistor with phase difference of $\tan^{-1}\left(\frac{i_q^e}{i_d^e}\right)$ between the voltage and this virtual single generator. 
