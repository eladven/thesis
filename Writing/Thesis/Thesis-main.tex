...
\Title{\textbf{Stability of coupled synchronous generators}}
...
respect to a reference angle, see Figure \ref{fig:structOfSG}. We denote its self
...
center of the star (see Figure \ref{fig:structOfSG}) and $v^n=[v_s\ v_s\ v_s]^\top$.
...
\centering \includegraphics[width=12cm]{SGStructure.eps}
...
i_{q}
\end{array}\right]
\\&-\frac{m}{L_{s}}\left[\begin{array}{c}
...
v_{q}
\end{array}\right].
\end{aligned}\label{eq:idiqDynamicsWithExIf}
\end{equation}
...
of single SG, if we ignore the dynamics of the rotor angle $\theta$.
...
\section{TICSG modeling\label{sec:TICSGModeling}}
...
%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%
%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%
\chapter{The equilibrium points  \label{cha:equivalence_pont}}
In this chapter, we study the equilibrium points for both the FOM model and the TICSG system.

\section{The Equilibrium points of FOM model.}
...
Section \ref{sec:TICSGModeling} that sometimes we prefer to consider the angle
...
\cite{AndrieuJayawardhanaPraly}.  Still at the general
...
Since condition \eqref{P_C1} holds, Theorem \ref{theorem:TICSGSync}
implies that the TICSG system has the TULES-NL property. Therefore
there exist strictly positive real numbers $r$, $k$ and $\l$ such for
all $(e_0,x_0,t)$ where $\left| e_0 \right| \leq r$, $e(t)$ satisfies
$\left|e(t)\right|\leq k\left|e_0\right| exp(-\l t)$. Thus, $\Emscr$
is exponentially stable for the TICSG system, locally in $e$,
uniformly in $x$.
\end{proof}
...
\begin{figure}[h]
\vspace{-15mm}
\includegraphics[scale=0.85]{OutROATICSGSimulation}
...
\begin{figure}[h]
\vspace{-5mm}
\includegraphics[scale=0.85]{NonStableTICSGSimulation}
...
\section{Proof that for the TICSG system, rank$\left[M|n\right]=3$, assuming that  $\sin\delta_{0}=0$ \label{chap:AppendixProof}}
