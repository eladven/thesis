...
exponentially stable (see for instance Theorum 5.17, p.~195 in \cite{Sastry}).
...
ELAD, check the units above, some were given wrong, correct all the 
later ones ???
...
ELAD, please write current, not Current.

ELAD, in the second plot below, could you write $\omega$ in place
      of w ???

ELAD, in the third plot below, could you change scale so that we 
see more of the currents ??? Increase the scale by about 2 times.

\vspace{-4mm}
%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%
\begin{figure}[ht] 
\includegraphics[scale=0.9]{5KWTICSGSimulation} \vspace{-8mm}
...
are $J=0.19$ {[}$kgm^{2}${]}, $D_{p}=1.7$ {[}$J/sec${]}, $R_{s}=0.152$
{[}$\Omega]$, $R_{L}=18.862$ {[}$\Omega]$, $L_{s}=0.41$ {[}$mH${]},
$mi_{f}=1.05$ {[}$Vsec]$, $T_{m}=543.2031$ {[}$N/m${]}.

ELAD, corrections are needed above ???

ELAD, why is J here slightly less than in the previous simulation ??? 
Could they not be equal? It makes the reader suspect that we are doing
something strange by playing with J.
...
not hold. The parameters for this simulation are $J=0.19$
{[}$kgm^{2}${]}, $D_{p}=1.7$ {[}$J/sec${]}, $R_{s}=0.152$
