...
In order to use the four order model of single SG \eqref{eq:SingleSGDynamics} in this system, we apply  Park transformation \eqref{eq:ParkTransformation}, on the terminal voltages: $\left[ v_{di}\ v_{qi}\  v_{0i} \right]^\top=U(\theta_{i})\left[v_{ai}\ v_{bi}\ v_{ci}  \right]^\top$,  and currents $\left[v_{di}\ v_{qi}\  v_{0i} \right]^\top=U(\theta_{i})\left[v_{ai}\ v_{bi}\ v_{ci}  \right]^\top$ where $i \in \{1,2\}$.
...
An expression for the voltage at each SG terminal which is the load voltage is:
