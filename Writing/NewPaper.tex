...
In this section, we study the equilibrium points of the TICSG system. First, we show that  if there exist equilibrium points, they must located on the synchronization manifold i.e. any equilibrium point $\left[z_1^e \ z_2^e \ \delta^e \right]^\top$ must satisfy $z_1^e=z_2^e$ and $\delta^e=0$ or $\delta^e = \pi$   (modulo $2\pi$). Second, we show that the TICSG system has at list one equilibrium point with zero phase (i.e with $\delta^e=0$).
...
That shows that the TICSG system has at list one equilibrium point  that located on the synchronization manifold 
$\epsilon=\{ \left[z^e \ z^e \ \delta^e  \right]^\top   |  \delta^e= 0\}$.

Note, that the anti synchronization manifold  $\epsilon '=\{ \left[z^e \ z^e \ \delta^e  \right]^\top   |  \delta^e=\pi \}$ may also have equilibrium points.
%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%

\section{Synchronization}

In this section, we show that the synchronization manifold of the TICSG system, is locally exponentially stable.
...
In order to show synchronization, it is convenient denote:
...
and the previous  equation can rewrite as: 
$$
...
We note that $F(0,x)=0$, which shows that the manifold
...
Again, because the following equation  
$$
(a+b)(x+y)+(a-b)(x-y)=2(ax+by)
$$
holds for any $x$, $y$, $a$ and $b$, it implies that:
...
\subsection{Globally exponentially stability of the TICSG system on the synchronization manifold }

Here, we provide sufficient conditions for that the equilibrium
point which located on the manifold $\epsilon=\left\{ \left[ x\ e \right]^\top \in\mathbb{R}^{7}|e=0\right\} $ is a globally
exponential stable equilibrium point, for any trajectory that is contained in the $\epsilon$ manifold. Because we showed that  $\epsilon$ is invariant manifold, it means that any trajectory  start on this manifold, remains inside it.

The dynamics of these trajectories is:
$$
\dot{\tilde{x}}=\tilde{G}(\tilde{x})=G(0,\tilde{x})=
 \Lambda^{-1} A\left( x_\omega \right) \tilde{x}+\Lambda^{-1} B 
$$

Where denote:
$$
A\left( \tilde{x}_\omega \right)=A_x\left(0, x_\omega \right) =  
 \left[\begin{array}{ccc}
...
Because the TICSG system has at list one equilibrium point on $\epsilon$, it means that $\tilde{G}(\tilde{x})$ has at list one equilibrium point 
$\tilde{x}^{e}=\left[\tilde{x}_{d}^{e}\ \tilde{x}_{q}^{e}\ \tilde{x}_{\omega}^{e} \right]^\top = 2z^e$:

\begin{equation}
\tilde{G}(x^{e})=0\rightarrow\varLambda^{-1}A(\tilde{x}_{\omega}^{e})\tilde{x}^{e}+\varLambda^{-1}\left[\begin{array}{c}
0\\
0\\
2T_{m}
...
We Perform coordinates shift , in order to make the origin into an equilibrium point.
$$
...
Calculate the derivative of the Lyponv function over  time: 
$$
...
All eigenvalues are real numbers. The condition for negative eigenvalues is:

\begin{equation}
16 R_TD_{p}>L_{s}^{2}\left(\left(x_{q}^{e}\right)^{2}+\left(x_{d}^{e}\right)^{2}\right)\label{eq:condition_for_stable_G}
\end{equation}

If  condition \eqref{eq:condition_for_stable_G} holds, then $\dot{V}(\hat{x})<-min\left(\left|\lambda_{i}\right|\right)|\hat{x}|^{2},i\in\{1,2,3\}$.
This proves that this system is globally exponentially stable. It means that $\tilde{x}^{e}$ is the only equilibrium point of $\tilde{G}(\tilde{x})$,
hence the original system has exactly one equilibrium point on the the $\epsilon$ manifold.
...
The requirement (2) at \cite{AndrieuJayawardhanaPraly} namely that matrix \eqref{eq:div_of_F} should by uniformly bounded  on the $\epsilon$i manifold is not satisfied, because of the presences of $x_{w},x_{q},x_{d}$ as elements in the matrix. However, it can be shown by considerations  that regardless
of the initial state, the state of this system is always contained in a certain bounded set (that depends on the initial state)
it implies that $\frac{\partial F(}{\partial e}(0,x)$ is clearly bounded in this bounded subset of the state space.

The requirement (3) in \cite{AndrieuJayawardhanaPraly}  satisfies
...
L.~Praly, \m Transverse exponential stability and applications, submitted in 2016.
