...
In this section, we show the relation between the ISE and the FOM. We start with the FOM, and by the following process, we get the ISE model. We separate the state variables into a fast variables and a slow 
variables, in order to use singular perturbation analysis (see for
instance \cite{Khalil}). Now the structure of the system is:
$$ \left\{ \begin{array}{c} \dot{x} \m=\m f(x,y,\e)\\
   \e\dot{y} \m=\m g(x,y,\e) \end{array}\right.$$
where $x$ is the vector of slow state variables, $y$ is the vector of
fast state variables and $\e>0$ is a small parameter.
