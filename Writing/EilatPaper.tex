...
and basically remained very similar until today. The grid is an 
enormously complex nonlinear and randonly varying system for which 
rigorous analysis is impossible. Many techniques and
models that have been developed to analyze the stability of the
grid, using rigorous modelling techniques but also practical shortcuts
and simplifying assumptions driven by experience and observations, see
for instance \cite{Kundur}, \cite{GrSt2014}, \cite{SauerPai1998},
\cite{GOBS:03}, \cite{DoBull2012}.

In recent years, due to the increasing penetration of renewable energy
resources, which require power converters and produce an intermittent
power output, it is not clear whether the traditional models and
methods for controlling the power grid will succeed to control it
\cite{ZhongWeiss2011,DePersiVanDerSchaft2016}. Therefore, there is an
increasing interest in the fundamental mathematical models and
stability analysis for the electrical grid.
...
when we model a large network. The electric grid to such a complicated
nonlinear and time-varying system that any attempt to prove its
stability analytically is hopeless. Stability analysis is usually done
...
only the rotor angle, and the rotor angular velocity as relevant state
