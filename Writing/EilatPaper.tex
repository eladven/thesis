...
of a single SG with constant field current. 
...
\end{equation}
We refer this fourth order nonlinear dynamical system of one SG connected to an infinte bus as the {\em fourth order model} (FOM).

%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%**********%%%%%%%%%%
\section{The improved swing equation}

In this section, we show the relation between the ISE and the FOM. We start with the FOM, and by reduction process, we get the ISE model. 
We start by applying the singular perturbation analysis (see for
instance \cite{Khalil}). The FOM model \rfb{eq:SGDynamicsInfBus} has the following structure:  $$\dot{z} = F(z) $$ Because some of it state variables are multiplied by small parameter ($L_s$), We represent this dynamical system in the following structure:
$$ \left\{ \begin{array}{c} \dot{x} \m=\m f(x,y,\e)\\
   \e\dot{y} \m=\m g(x,y,\e) \end{array}\right.$$

By doing this, we separate the state variables into a fast variables and a slow variables. $x$ is the vector of the slow state variables, and $y$ is the vector of the fast state variables and $\e>0$ is a small parameter.

Assuming that $L_s$ is very small with compared to the other parameters, we
 define $\e=L_s$, $x=\left[\begin{array}{c} \o\\ \delta\end{array}
\right]$ and $y=\left[\begin{array}{c} i_d\\ i_q \end{array}\right]$.
By taking the first and second lines from \rfb{eq:SGDynamicsInfBus}, we have
$$ g(x,y,\e) \m=\m \left[\nm \begin{array}{cc} -R_s & \o L_s\\ -\o 
   L_s & -R_s \end{array}\nm\right] \left[\nm \begin{array}{c} i_d\\ 
   \i_q \end{array} \nm\right]+\left[\begin{array}{c} V\sin\delta\\ 
   V\cos\delta - mi_f\o \end{array}\right].$$
Because our assumption  that $\e$ is very small, meaning that $\e\dot{y}=g(x,y,\e)$
is much faster than $\dot{x}=f(x,y,\e)$, and it will reach its equilibrium (assuming that the equilibrium point exist and attractive)
almost instantly related to $x$.  The equilibrium point
...
\hat{y}  =
\left[\begin{array}{cc}
\frac{R_{s}V\sin(\delta)-L_{s}\omega\left(mi_{f}\omega-
V\cos(\delta)\right)}{L_{s}^{2}\omega^{2}+R_{s}^{2}}\\
...
, and assuming that these dynamics are close to the equilibrium, namely $\omega\simeq\omega_{g}$ then: 
...
Using the previous equation, substitute $\hat{y}$ in $\dot{x}=f(x,y,\epsilon)$ to have a
reduced model:
\[
\left\{ \begin{array}{c}
...
In this section we present simulations which demonstrates that the ISE model is a good reduction for many cases. We also show that there are cases in which there is a mismatch between the behavior suggested by the ISE model and the FOM. In each simulation result, the behavior of $i_d$ and $i_q$ over time is described for both the FOM
and the ISE models. 
Note that for the ISE model, we use \eqref{eq:ISEInfBusEstimatedCurrents} to estimate these currents.  The frequency over time of these two models  models, and the phase over time of the FOM and the ISE models are also shown .
...
As shown in Figure \ref{fig:InfBusOne5KWSG}, simulations indicates that
...
We have shown the relation between the FOM of SG connected
to an infinite bus and the ISE model. Simulations
are carried out to show that the FOM gives rise to
a behavior which doesn't match the one suggested by the ISE. 
