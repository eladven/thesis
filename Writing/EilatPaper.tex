...
The power absorbed from the prime mover can be expressed approximately as
...
many different approximations to derive it. It is normally assumed that $P_m$ is constant, but this is not necessary. 

Let us see what happens if we do not use the crude approximation \eqref{eq:tourqeApproximation} and instead return to \eqref{eq:ISWWithConstTm}, divide the first equation by $\omega$ and substitute $\dot{\omega} = \ddot{\delta}$, we then obtain 
$$
J\ddot{\delta}+D_p \dot{\delta} + \frac{mi_f V \sin \delta}{L_s \left(\omega_g + \dot{\delta}\right)}=T_m - D_p \omega_g
$$
This is an ugly nonlinear ODE, but (if $R_s=0$) its equilibrium points are exactly the same as for the FOM and if we approximate $\omega_g+\dot{\delta} \approx \omega_g$, then we get the classical swing equation.
...
time for these two models. In all simulations , variables with a hat correspond to the reduced model. All simulations assume constant $P_m$.
