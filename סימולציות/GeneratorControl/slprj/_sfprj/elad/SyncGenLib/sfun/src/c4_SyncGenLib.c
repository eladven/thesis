...
static void initialize_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct
  *chartInstance);
static void initialize_params_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct
  *chartInstance);
static void enable_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct *chartInstance);
static void disable_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct *chartInstance);
static void c4_update_debugger_state_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct *
  chartInstance);
static void set_sim_state_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct
  *chartInstance, const mxArray *c4_st);
static void finalize_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct *chartInstance);
static void sf_c4_SyncGenLib(SFc4_SyncGenLibInstanceStruct *chartInstance);
static void initSimStructsc4_SyncGenLib(SFc4_SyncGenLibInstanceStruct
  *chartInstance);
static void registerMessagesc4_SyncGenLib(SFc4_SyncGenLibInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c4_machineNumber, uint32_T
  c4_chartNumber);
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData);
static real_T c4_emlrt_marshallIn(SFc4_SyncGenLibInstanceStruct *chartInstance,
  const mxArray *c4_c, const char_T *c4_identifier);
static real_T c4_b_emlrt_marshallIn(SFc4_SyncGenLibInstanceStruct *chartInstance,
  const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void c4_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData);
static const mxArray *c4_b_sf_marshallOut(void *chartInstanceVoid, void
  *c4_inData);
static int32_T c4_c_emlrt_marshallIn(SFc4_SyncGenLibInstanceStruct
  *chartInstance, const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void c4_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData);
static uint8_T c4_d_emlrt_marshallIn(SFc4_SyncGenLibInstanceStruct
  *chartInstance, const mxArray *c4_b_is_active_c4_SyncGenLib, const char_T
  *c4_identifier);
static uint8_T c4_e_emlrt_marshallIn(SFc4_SyncGenLibInstanceStruct
  *chartInstance, const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void init_dsm_address_info(SFc4_SyncGenLibInstanceStruct *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c4_is_active_c4_SyncGenLib = 0U;
}
  ...
  chartInstance)
{
  ...
  c4_d_hoistedGlobal = chartInstance->c4_is_active_c4_SyncGenLib;
  ...
  *chartInstance, const mxArray *c4_st)
{
  ...
  *c4_c = c4_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c4_u, 2)),
    "c");
  chartInstance->c4_is_active_c4_SyncGenLib = c4_d_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c4_u, 3)),
     "is_active_c4_SyncGenLib");
  sf_mex_destroy(&c4_u);
  c4_update_debugger_state_c4_SyncGenLib(chartInstance);
...
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData)
{
  ...
  real_T c4_u;
  const mxArray *c4_y = NULL;
  SFc4_SyncGenLibInstanceStruct *chartInstance;
  chartInstance = (SFc4_SyncGenLibInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c4_SyncGenLib_get_eml_resolved_functions_info(void)
{
  ...
  *c4_identifier)
{
  ...
  c4_y = c4_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c4_b_is_active_c4_SyncGenLib), &c4_thisId);
  sf_mex_destroy(&c4_b_is_active_c4_SyncGenLib);
  return c4_y;
}
...
void sf_c4_SyncGenLib_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c4_SyncGenLib_get_autoinheritance_info(void)
{
...
mxArray *sf_c4_SyncGenLib_third_party_uses_info(void)
{
  mxArray * mxcell3p = mxCreateCellMatrix(1,0);
  return(mxcell3p);
}

static const mxArray *sf_get_sim_state_info_c4_SyncGenLib(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x4'type','srcId','name','auxInfo'{{M[1],M[5],T\"a\",},{M[1],M[6],T\"b\",},{M[1],M[7],T\"c\",},{M[8],M[0],T\"is_active_c4_SyncGenLib\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 4, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c4_SyncGenLib_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc4_SyncGenLibInstanceStruct *chartInstance;
    chartInstance = (SFc4_SyncGenLibInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c4_SyncGenLib(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar
    )->S,0);
  initialize_params_c4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*)
    chartInstanceVar);
  initialize_c4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c4_SyncGenLib(void *chartInstanceVar)
{
  enable_c4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c4_SyncGenLib(void *chartInstanceVar)
{
  disable_c4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c4_SyncGenLib(void *chartInstanceVar)
{
  sf_c4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c4_SyncGenLib(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c4_SyncGenLib
    ((SFc4_SyncGenLibInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c4_SyncGenLib();/* state var info */
...
extern void sf_internal_set_sim_state_c4_SyncGenLib(SimStruct* S, const mxArray *
  st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c4_SyncGenLib(SimStruct* S)
{
  return sf_internal_get_sim_state_c4_SyncGenLib(S);
}

static void sf_opaque_set_sim_state_c4_SyncGenLib(SimStruct* S, const mxArray
  *st)
{
  sf_internal_set_sim_state_c4_SyncGenLib(S, st);
}

static void sf_opaque_terminate_c4_SyncGenLib(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
      unload_SyncGenLib_optimization_info();
    }

    finalize_c4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc4_SyncGenLib((SFc4_SyncGenLibInstanceStruct*) chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c4_SyncGenLib(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c4_SyncGenLib(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
...
static void mdlRTW_c4_SyncGenLib(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c4_SyncGenLib(SimStruct *S)
{
  SFc4_SyncGenLibInstanceStruct *chartInstance;
  chartInstance = (SFc4_SyncGenLibInstanceStruct *)utMalloc(sizeof
    (SFc4_SyncGenLibInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc4_SyncGenLibInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c4_SyncGenLib;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c4_SyncGenLib;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c4_SyncGenLib;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c4_SyncGenLib;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c4_SyncGenLib;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c4_SyncGenLib;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c4_SyncGenLib;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c4_SyncGenLib;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c4_SyncGenLib;
  chartInstance->chartInfo.mdlStart = mdlStart_c4_SyncGenLib;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c4_SyncGenLib;
...
void c4_SyncGenLib_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c4_SyncGenLib(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c4_SyncGenLib(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c4_SyncGenLib(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c4_SyncGenLib_method_dispatcher.\n"
